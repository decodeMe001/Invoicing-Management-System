...
class Crud_model extends CI_Model {
    ...
    public function insertOrder($data)
    {
		try {
			...
			foreach($this->cart->contents() as $items)
			{
				...
				$get_product = $this->db->get_where('store_product', array('id' => (int)trim($items["id"]), 'title' => trim($items["name"])))->result_array();
				foreach($get_product as $item){
					$qty_left = (int)$item['qty_in_stock'] - (int)trim($items["qty"]);
					$this->db->where(array('id' => (int)trim($items["id"]), 'title' => trim($items["name"])));
    ...
    public function insertOrderWithoutSlip($data)
    {
		$total_profit_margin = 0;
		$data1 = array(
			'order_no'      =>  trim($data["order_no"]),

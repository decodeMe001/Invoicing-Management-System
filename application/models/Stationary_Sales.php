...
ob_start();
class Stationary_Sales extends CI_Model {
    ...
    public function insert($data)
    {
	   ...
	   $q = $this->db->insert_string('stationary_sales', $data1);
	   $this->db->query($q);
      ...
      for($count=0; $count < $_POST["total_item"]; $count++)
      {
		...
		$q2 = $this->db->insert_string('stationary_sales_order', $data2);
        ...
        $this->db->update('stationary_sales', $data3);
	}
	...
	public function update($id)
    {
        ...
        $this->db->delete('stationary_sales_order');
		for($count=0; $count < $_POST["total_item"]; $count++)
		{
		...
		$this->db->where('id', $id);
        $this->db->update('stationary_sales', $data1);
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete('stationary_sales');
		$this->db->where('sales_id', $id);
        $this->db->delete('stationary_sales_order');
    }

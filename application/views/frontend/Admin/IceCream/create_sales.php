...
<form method="post" id="invoice_form" action="<?php echo base_url();?>admin/ice_cream/create">
  <div class="table-responsive">
	<table class="table table-bordered table-striped display">
	  <tr>
		<center><h2 style="margin-top:10.5px;color:2ef2ef;">Create Ice-Cream Invoice</h2></center>
	  </tr>
		<tr>
			<div class="row">
				<div class="col-md-5">
					<b>VENDOR (BILL TO)</b>
					<select name="vendor_id" id="vendor_id" class="form-control input-sm" required>
						<option value="" <?php if($vendor) { foreach($vendor as $value) { ?>>--select--</option>
						<option value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
					<?php } } else { ?>
						<option>No vendor data entry</option>
					<?php } ?>
					</select>
				</div>
				<div class="col-md-5">
				...
				<tr>
				  <td><span id="sr_no">1</span></td>
					<td>
						<select name="category[]" id="category1" class="form-control input-sm category" >
					...
					<td>
						<select name="product_id[]" id="product_id1" class="form-control input-sm product_id" >
		...
		<tr>
			<div class="row" style="margin: 10px 15px;">
				...
				<input type="number" name="balance" id="balance" class="form-control col-md-8 input-sm balance" placeholder="&#8358;" readonly required />
			</div>
		</tr>
		<tr>
			<div class="row" style="margin: 10px 15px;">
...
<script type="text/javascript">
      $(document).ready(function(){
        ...
        $(document).on('click', '#add_row', function(){
			...
			html_code += '<td><select name="product_id[]" id="product_id'+count+'" class="form-control input-sm product_id" required><option value="" <?php if($total_product_rows > 0) { ?>>--select--</option><?php foreach($get_product_array as $product) { ?><option value="<?= $product['id'] ?>" ><?= $product['product_name'] ?></option><?php } }else { ?><option value="">No entry yet</option><?php } ?></select></td>';

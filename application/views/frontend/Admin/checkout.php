			...
			if($total_rows > 0)
			{
				...
				foreach ($product_data as $row)  { 
					...
					<tr>
						...
						<td>&#8358;<?=number_format($row["selling_price"] - $row["market_price"], 2, '.', ',');?></td>
...
<script type="text/javascript">
	...
	function showPrintDialog(event) {
			event.preventDefault(); // Prevent form submission
			$('#checkButton').prop("disabled", true);
			// Show the dialog box
			if (confirm("Do you want to print receipt?")) {
				// User chooses to print
				submitFormForReceipt();
			} else {
				// User chooses not to print
				submitFormWithoutReceipt();
			}
		}
	
		
		function submitFormForReceipt() {
			var formData = $('#invoice_form').serialize(); // Serialize form data
			$.ajax({
				type: 'POST',
				url: "<?=base_url(); ?>admin/checkout/print_slip",
				data: formData,
	   ...
	   function submitFormWithoutReceipt() {
			var formData = $('#invoice_form').serialize(); // Serialize form data
			$.ajax({
				type: 'POST',
				url: "<?=base_url(); ?>admin/checkout/no_slip",
				data: formData,
				success: function(response) {
					location.reload();
				},

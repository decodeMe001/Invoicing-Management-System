<?php
$result1 = $this->db->get_where('ice_cream_sales', array('id' => $id))->result_array();
...
<form method="post" id="invoice_form" action="<?php echo base_url();?>admin/ice_cream/update/<?=$id ?>">
	  ...
	  <table class="table table-bordered">
		...
		<tr>
			<div class="row">
				<div class="col-md-5">
					<b>VENDOR (BILL TO)</b>
					<input type="text" name="vendor_id" id="vendor_id" class="form-control input-sm" value="<?=$row['vendor_id'] ?>" readonly />
				</div>
				<div class="col-md-5">
					<b>Payment Date</b>
				...
				<tr></tr>
				<?php
		  			$result2 = $this->db->get_where('ice_cream_sales_order', array('sales_id' => $id))->result_array();
                    $m = 0;
				...
				<tr>
				  <td><span id="sr_no"><?php echo $m; ?></span></td>
					...
					<td>
						<select name="product_id[]" id="product_id<?php echo $m; ?>" class="form-control input-sm product_id" >
					...
					<td>
						<input type="text" name="item_name[]" id="item_name<?php echo $m; ?>" data-srno="<?php echo $m; ?>" class="form-control input-sm item_name" required />
					</td>
					<td>
						<input type="number" name="order_item_quantity[]" id="order_item_quantity<?php echo $m; ?>" data-srno="<?php echo $m; ?>" value="<?=$sub_row["order_item_quantity"];?>" class="form-control input-sm order_item_quantity" required />
					</td>
					<td>
						<input type="number" name="unit_price[]" id="unit_price<?php echo $m; ?>" placeholder="&#8358;" data-srno="<?php echo $m; ?>" class="form-control input-sm unit_price" value="<?=$sub_row['unit_price']?>" required readonly />
					</td>
					<td>
						<input type="number" name="selling_price[]" id="selling_price<?php echo $m; ?>" placeholder="&#8358;" data-srno="<?php echo $m; ?>" class="form-control input-sm selling_price" value="<?=$sub_row['selling_price']?>" required readonly />
					</td>
					<td>
						<input type="number" name="total_cost_price[]" id="total_cost_price<?php echo $m; ?>" placeholder="&#8358;" data-srno="<?php echo $m; ?>" class="form-control input-sm total_cost_price" required />
					</td>
					<td>
						<input type="number" name="total_selling_price[]" id="total_selling_price<?php echo $m; ?>" placeholder="&#8358;" data-srno="<?php echo $m; ?>" class="form-control input-sm total_selling_price" required />
						<input type="hidden" name="qty_left[]" id="qty_left<?php echo $m; ?>" class="form-control input-sm" />
						<input type="hidden" name="qty_stocked[]" id="qty_stocked<?php echo $m; ?>" class="form-control input-sm" />
					</td>
				</tr>
		  </table>
		</tr>
		...
		<tr>
			<div class="row" style="margin: 10px 15px;">
				...
				<input type="number" name="balance" id="balance" class="form-control col-md-8 input-sm balance" value="<?=$row["balance"];?>" required />
			</div>
		</tr>
		<tr>
			<div class="row" style="margin: 10px 15px;">
...
<script type="text/javascript">
	...
	$('#create_invoice').click(function(e){
		e.preventDefault();
		if($.trim($('#vendor_id').val()).length == 0)
		{
